...
5. AgentLab AI settings moved from provider level to top-level settings for easier access
...
3. Auto-enable web search when using AgentLab AI
...
6. AgentLab AI image generation now supports uploading reference images
...
3. Fixed AgentLab AI error message display
4. Added initialization process log display

### v1.12.0 - 2025.04.24

1. AgentLab AI supports Gemini multimodal output
...
1. Introduced Web Browsing capability. AgentLab AI Models and Gemini 2.0 flash(API) can now be enabled to access real-time internet information, providing responses with cited sources
2. Added first-token latency display for message generation, toggleable in settings
3. Enhanced image preprocessing capabilities. You can now send images in more formats (e.g., svg, gif) - AgentLab automatically converts them to model-compatible formats and adjusts dimensions to meet model requirements
4. Double-click thread title to quickly access the thread list
5. Improved compatibility for custom model provider configurations - AgentLab now automatically corrects and handles common configuration errors
...
1. Now you can send web links. AgentLab will automatically fetch webpage content and include it in the chat context. Works with all models.
2. Now you can send text files to any models. AgentLab will parse file content locally and include it in the chat context.
...
1. AgentLab AI service users can now choose from a wider range of models
...
1. AgentLab AI 3.5 now supports sending images.
...
- Enabled access to local Ollama services within the AgentLab web version.
...
3. Text copied from AgentLab will be copied to the clipboard as plain text without background color — a longstanding minor bug that has finally been resolved.
...
- Added an image generation feature (Image Creator); you can now generate images within AgentLab, powered by the Dall-E-3 model.
...
- Built-in AI service "AgentLab AI" - ready to use out of the box with fast, hassle-free setup.
