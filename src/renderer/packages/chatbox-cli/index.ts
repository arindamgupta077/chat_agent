...
function commandHelp(domain?: string): Record<string, unknown> {
  ...
  return {
    ...
    notes: [
      ...
      'Settings access is read-only. Guide users to AgentLab Settings for changes.',
      'Starting image generation requires explicit user approval in AgentLab.',
      ...
      'AgentLab shows generated images to the user inline at the generate tool call. Never repeat them in replies.',
...
export function getChatboxCliDescription(): string {
  return `
### AgentLab Virtual CLI
Use \`chatbox_cli\` for AgentLab account status, read-only settings, conversation history, and image generation.
...
- Read safe settings: \`["settings", "list"]\`, \`["settings", "get", "appearance.theme"]\`. Settings cannot be changed through this tool; guide the user to the returned AgentLab Settings location to change them manually.
- Start background image work: \`["image", "generate", "--prompt", "..."]\`. AgentLab shows a localized approval card before submitting the potentially billable request.
...
- After an accepted image task, end the turn and wait for AgentLab's automated callback. Never poll \`image status\`; use it only after a callback, on explicit user request, or for recovery diagnostics.
- AgentLab shows generated images to the user inline at the generate tool call. Never repeat them in replies: no markdown images and no image links.
